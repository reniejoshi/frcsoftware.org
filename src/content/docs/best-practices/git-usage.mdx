...
| :--------------------------------------------------------------------------------------- | :---------------------------------------------------------------------- |
| Completes a single task                                                                  | Includes logic changes, formatting fixes, and refactors in one commit   |
| Uses the imperative mood: "Refactor swerve drive logic"                                  | Past tense, lowercase, and punctuated: "refactored swerve drive logic." |
| Specific and provides context: "Fix swerve drive limiter bug for reliable drive control" | Vague: "Fix bug" or "WIP"                                               |
...
To commit from the command line, first add the desired changes to the staging area using `git add <file>`.
`git add .` adds all of your changes.
Use `git commit -m "message"` to create a commit.
It is a good practice to subsequently run `git push` to sync your changes to the remote repository.
...
You should also push to your branch frequently to back up your work and make it visible to other programmers using `git push.`
